...
public class Main {
    ...
    public static void printStudioWithMoviesGreaterThan(Studio[] studios, int x) {
        for (Studio s : studios) {
            if (s.getFilme().length > x) {
                System.out.println(s.getNume());
            }
        }
    }

    public static void printStudioWithActor(Studio[] studios, String desiredActorName) {
        for (Studio s : studios) {
            for (Film f : s.getFilme()) {
                for (Actor a: f.getActori()) {
                    if (a.getNume().equals(desiredActorName)) {
                        System.out.println(s.getNume());
                    }
                }
            }
    ...
    public static void printMoviesWithActorAge(Studio[] studios, Integer age) {
        for (Studio s : studios) {
            for (Film f : s.getFilme()) {
                for (Actor a: f.getActori()) {
                    if (a.getVarsta() > age) {
    ...
    public static void main(String[] args) {
        ...
        printStudioWithMoviesGreaterThan(studioDatabase, 2);
        printStudioWithActor(studioDatabase,"actor cu 2 oscaruri");
        printMoviesWithActorAge(studioDatabase, 50);
